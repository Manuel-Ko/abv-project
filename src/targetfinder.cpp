...
const cv::Size TargetFinder::SHOW_WINDOWSIZE = cv::Size(1238,697);
...
void TargetFinder::extractAllTargetRings(const std::vector<std::vector<cv::Point>>& coarseBullsEyes, cv::Mat& p_debugMat)
{
    ...
    for(size_t i = 0; i < contours.size(); ++i)
    {
		...
		const int maxSideLength = std::max(bullsEyeMask.cols,bullsEyeMask.rows);
...
//    cv::cvtColor(roiImage, roi_gray, CV_BGR2GRAY);
    ...
    /*cv::dilate(thresholdImage,thresholdImage,element2);
    cv::erode(thresholdImage,thresholdImage,element2);*/
       ...
       /* for(int j = 0; j < rings.size(); ++j)
        ...
        if(DRAW_BULLERHOLES)
        {
            ...
            for(size_t j = 0; j < holes.size(); ++j)
            {
                ...
                cv::putText(p_drawOn,score,holeCenter /*- cv::Point(holes[j][2],holes[j][2])*/,CV_FONT_HERSHEY_DUPLEX,TEXT_SCALE,cv::Scalar(0,0,0),2);
            }
        }
    }
}
